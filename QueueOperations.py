...
class DQ:
    ...
    def calculate_ti(self):
        if self.λ > self.μ:
            Tik = symbols('Tik')
            eqn = Eq(((self.λ * Tik) - ((self.μ * Tik) - (self.μ / self.λ))), self.k)
            Ti = solve(eqn)[0]
            Tik = Ti
            kTi = self.k
            while kTi == self.k:
                ...
                Tik = Ti - (1 / self.λ)
                x1, x2 = symbols('x1, x2')
                x1 = solve(Eq(self.λ * Tik, x1))[0]
                x2 = solve(Eq((self.μ * Tik) - (self.μ / self.λ), x2))[0]
                kTi = math.floor(x1)-math.floor(x2)
    ...
    def number_Of_customers(self, t):
        if t < (1 / self.λ) and self.M == 0:
            return 0
        elif self.λ > self.μ:
            if t < self.ti:
                x1, x2 = symbols('x1, x2')
                x1 = solve(Eq(self.λ * t, x1))[0]
                x2 = solve(Eq((self.μ * t) - (self.μ / self.λ), x2))[0]
                return math.floor(x1) - math.floor(x2)
                # return self.M + math.floor(self.λ * t) - math.floor((self.μ * t) - (self.μ / self.λ))
            else:
                if self.is_departure(t):
                    ...
                    # no one left or arrived n(t) stays the same
                    return self.number_Of_customers(t-1)
        else:
            if self.λ == self.μ:
                if self.M == 0:
                    return 1
                ...
                if t < self.ti:
                    ...
                    if (t % (1/self.λ)) < (1/self.μ):
                        return 1
                    else:
                        return 0

    def is_departure(self, t):
        return (t % (1/self.μ)) == (1/self.λ)
